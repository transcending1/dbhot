...
class CourseSubCategory(models.Model):
    ...
    category = models.ForeignKey("CourseCategory", on_delete="1")
    name = models.CharField(max_length=64, unique=True)
...
class Scholarship(models.Model):
    """学位课程奖学金"""
    degree_course = models.ForeignKey("DegreeCourse", on_delete="1")
...
class Course(models.Model):
    ...
    sub_category = models.ForeignKey("CourseSubCategory", on_delete="1")  #

    course_type_choices = ((0, '付费'), (1, 'VIP专享'), (2, '学位课程'))
    course_type = models.SmallIntegerField(choices=course_type_choices)

    degree_course = models.ForeignKey("DegreeCourse", blank=True, null=True, help_text="若是学位课程，此处关联学位表", on_delete="1")
...
class CourseDetail(models.Model):
    """课程详情页内容"""
    course = models.OneToOneField("Course", on_delete="1")
...
class OftenAskedQuestion(models.Model):
    """常见问题"""
    content_type = models.ForeignKey(ContentType, on_delete="1")  # 关联course or degree_course
...
class CourseOutline(models.Model):
    """课程大纲"""
    course_detail = models.ForeignKey("CourseDetail", on_delete="1")
...
class CourseChapter(models.Model):
    """课程章节"""
    course = models.ForeignKey("Course", related_name='coursechapters', on_delete='1')
...
class CourseSection(models.Model):
    """课时目录"""
    chapter = models.ForeignKey("CourseChapter", related_name='coursesections', on_delete="1")
    name = models.CharField(max_length=128)
...
class Homework(models.Model):
    chapter = models.ForeignKey("CourseChapter", on_delete="1")
...
class Article(models.Model):
    ...
    source = models.ForeignKey("ArticleSource", verbose_name="来源", on_delete="1")
...
class Collection(models.Model):
    """收藏"""
    content_type = models.ForeignKey(ContentType, on_delete="1")
    object_id = models.PositiveIntegerField()
    content_object = GenericForeignKey('content_type', 'object_id')

    account = models.ForeignKey("Account", on_delete="1")
...
class Comment(models.Model):
    """通用的评论表"""
    content_type = models.ForeignKey(ContentType, blank=True, null=True, verbose_name="类型", on_delete="1")
    object_id = models.PositiveIntegerField(blank=True, null=True)
    content_object = GenericForeignKey('content_type', 'object_id')

    p_node = models.ForeignKey("self", blank=True, null=True, verbose_name="父级评论", on_delete="1")
    content = models.TextField(max_length=1024)
    account = models.ForeignKey("Account", verbose_name="会员名", on_delete="1")
    disagree_number = models.IntegerField(default=0, verbose_name="踩")
    agree_number = models.IntegerField(default=0, verbose_name="赞同数")
    date = models.DateTimeField(auto_now_add=True)
...
class UserAuthToken(models.Model):
    ...
    用户Token表
    """
    user = models.OneToOneField(to="Account", on_delete="1")
